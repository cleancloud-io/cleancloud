...
def find_idle_vertex_endpoints(
    *,
    ...
    for location, ep_list in eligible_by_location.items():
        ...
        counts = _get_prediction_counts_batch(
        ...
        for ep_info in ep_list:
            ...
            no_monitoring_data = endpoint_id_key not in counts
            ...
            effective_threshold = int(base_threshold * max(1.0, total_min_replicas_ep**0.5))
            ...
            # Safety guard: no monitoring data + no age info = too many unknowns to flag
            if no_monitoring_data and age_days is None:
                continue

            # Confidence based on traffic and age.
            # HIGH requires both: age meets threshold AND full observation window observed.
            if is_near_idle:
                # Some traffic exists — cap at MEDIUM regardless of age
            ...
            recommendations.append(
                "Switch to automaticResources if the workload is not latency-critical "
                "(scales to zero when idle, eliminating always-on compute cost)"
            )
            ...
            # GPU pricing varies by region — flag estimate as approximate
            cost_qualifier = " (us-central1 baseline)" if is_gpu else ""
            cost_note = f"~${monthly_cost:,.0f}/month{cost_qualifier}"
            ...
            if no_monitoring_data and not is_near_idle:
                signals.append(
                    "No prediction request data found in Cloud Monitoring — treated as zero traffic"
                )
...
def _list_endpoints(session: AuthorizedSession, project_id: str) -> list:
    ...
    Uses the locations/- wildcard supported by the Vertex AI API (AIP-131) to
    fetch endpoints from every region in a single paginated call sequence.
    Raises PermissionError on 403. Returns [] on 404 (API not enabled).
    """
    url = f"https://aiplatform.googleapis.com/v1/projects/{project_id}/locations/-/endpoints"
    endpoints = []
...
def _get_prediction_counts_batch(
    ...
    Returns {endpoint_id: total_request_count} — endpoints with no data points
    are absent from the dict (caller treats absence as zero traffic and adds a
    transparency signal). Returns None on any error; callers should skip all
    endpoints in the location (conservative fallback).
    """
    try:
        now = datetime.now(timezone.utc)
        ...
        for series in results:
            ...
            for point in series.points:
                ...
                counts[ep_id] = counts.get(ep_id, 0) + val
